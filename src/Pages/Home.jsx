...
import {IconChartDots2, IconLogout, IconMap2, IconUser} from '@tabler/icons-react'
...
function calculateSuccessProbability(averageRating, numberOfReviews) {
  ...
  const weightedSum = ratingWeight * normalizedRating + reviewsWeight * (numberOfReviews / 1000);
...
function calculateSuccessPercentageForArea(areaFilteredData) {
  ...
  const averageRating = areaFilteredData.reduce((sum, restaurant) => sum + restaurant.stars, 0) / areaFilteredData.length;
  const totalReviews = areaFilteredData.reduce((sum, restaurant) => sum + restaurant.numberOfReviews, 0);


  const normalizedRating = (averageRating / 5) * 100;

  const weightedSum = ratingWeight * normalizedRating + reviewsWeight * (totalReviews / 1000);
  const probability = 100 / (1 + Math.exp(-weightedSum));
    ...
    const { isLoaded } = useJsApiLoader({
      ...
      const handleMapClick = (event) => {
        ...
        setFilRest(restaurantsInRadius)
         ...
         const filteredRating = restaurantsInRadius.reduce((sum, restaurant) => sum + restaurant.stars, 0);
const averageRatingFiltered = filteredRating / restaurantsInRadius.length;
const mostReviewedRestaurant = restaurantsInRadius.reduce((maxReviews, restaurant) => {
  return restaurant.numberOfReviews > maxReviews.numberOfReviews ? restaurant : maxReviews;
...
const highestRatedRestaurant = restaurantsInRadius.reduce((maxRating, restaurant) => {
  return restaurant.stars > maxRating.stars ? restaurant : maxRating;
}, restaurantsInRadius[0]);

const lowestRatedRestaurant = restaurantsInRadius.reduce((minRating, restaurant) => {
  return restaurant.stars < minRating.stars ? restaurant : minRating;
}, restaurantsInRadius[0]);


const filteredReviews = restaurantsInRadius.reduce((sum, restaurant) => sum + restaurant.numberOfReviews, 0);
...
const highRatedRestaurantsCount = restaurantsInRadius.filter((restaurant) => restaurant.stars > 4.5).length;

const filteredReviewLength = restaurantsInRadius.reduce((sum, restaurant) => {
...
const averageReviewLengthFiltered =
  restaurantsInRadius.length > 0 ? filteredReviewLength / restaurantsInRadius.length : 0;
...
const catPercent = calculateCategoryPopularityPercentage(restaurantsInRadius)
...
setOpen(true)
  ...
  return (
    ...
    <ul className='flex flex-col space-y-8 items-center' >
        <li className='p-2 bg-[#FF385C] rounded-full' >
        ...
        <li className='absolute bottom-12 cursor-pointer' 
        onClick={() => {
          signOut()
          navigate('/login')
        }}
    ...
    <Modal open={open} onClose={() => setOpen(false)}>
        ...
        <div>
          ...
          stats?.mostPopularCategories?.map((o, i) => (
            <div className='flex flex-row items-center gap-3 mb-2' key={i} >
  ...
  <table className="w-full text-sm text-left rtl:text-right text-gray-500 ">
    ...
    Object.entries(stats?.categoryCounts).map(([key, value]) => (
        ...
        <div>
          <BarChartPopularCategories categoryCounts={stats?.categoryCounts || []} />
        </div>
         <div>
          <BubbleChartRatingsVsReviewsVsReviewLength restaurantData={filRes} />
        </div>
        <div>
          <DoughnutChartRatingDistribution restaurantData={filRes} />
        </div>
        <div>
          <HorizontalBarChartAverageReviewsPerCategory restaurantData={filRes} />
        </div>
        <div>
